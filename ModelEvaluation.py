...
class Bayes:
    ...
    def CI_Bayes(self, direction):
        ...
        for threshold_i in self.threshold:
            ...
            for item, j, a, b in zip(self.data, threshold_i, self.alpha, self.beta):
                ...
                CI = stats.beta.ppf(
                    [self.alphalevel / 2, 1 - self.alphalevel / 2], param_1, param_2
                ).round(4)
                res.append(
                    [
                        param_1,
                        ...
                        round(
                            (param_1 * param_2)
                            / (pow(param_1 + param_2, 2) * (param_1 + param_2 + 1)),
                            6,
                        ),
                        CI,
                        str(threshold_i),
            ...
            res = pd.DataFrame(
                ...
                columns=[
                    ...
                    f"{(1-self.alphalevel)*100}%-level Credible Interval",
                    "Threshold",
                ],
            )
    ...
    def Plot_Bayes(self, yrange=[0, 1, 0.05], width=1200, height=1200):
        CI_upper = [
            x[1] - y
            for x, y in zip(
                self.res[f"{(1-self.alphalevel)*100}%-level Credible Interval"],
                self.res["Expected Success Probability"],
            )
        ]
        CI_lower = [
            y - x[0]
        ...
        fig = px.bar(
            ...
            text=[
                str("{:.2%}".format(x))
                for x in self.res["Expected Success Probability"]
            ],
        )
        ...
        fig.update_layout(
            ...
            height=height,
            title=dict(
                text=f"Expected Success Probability with {(1-self.alphalevel)*100}% Confidence Level",
                x=0.5,
            ),
        )
        fig.show()
